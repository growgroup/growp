...
class GNav
{
     ...
     * @return string
     */
    public static function get_paging_nav($prev_text = "&laquo; Previous", $next_text = "Next &raquo;")
    {
    ...
    public static function _paginate_links( $args = '' ) {
        ...
        if ( $args['prev_next'] && $current && 1 < $current ) :
             ...
             */
            $page_links[] = '<li class="c-pagination__prev"><a class="prev page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $args['prev_text'] . '</a></li>';
        endif;
        for ( $n = 1; $n <= $total; $n++ ) :
            if ( $n == $current ) :
        ...
        if ( $args['prev_next'] && $current && ( $current < $total || -1 == $total ) ) :
            ...
            /** This filter is documented in wp-includes/general-template.php */
            $page_links[] = '<li class="c-pagination__next"><a class="next page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '">' . $args['next_text'] . '</a></li>';
        endif;
        switch ( $args['type'] ) {
            case 'array' :
                return $page_links;
